...
from .email_manager import fetch_new_emails, get_all_emails, get_email_summary, send_email_reply
import json
...
def finance_view(request):
    """Page du pôle finance"""
    if 'user_pole' not in request.session:
        return redirect('login')

    if request.session['user_pole'] != 'finance':
        return redirect('login')
...
def developpement_view(request):
    """Page du pôle développement"""
    if 'user_pole' not in request.session:
        return redirect('login')

    if request.session['user_pole'] != 'developpement':
        return redirect('login')

    return render(request, 'developpement.html', {
        'pole_name': 'Développement'
    })


def technique_view(request):
    """Page du pôle technique"""
    if 'user_pole' not in request.session:
        return redirect('login')

    if request.session['user_pole'] != 'technique':
        return redirect('login')
